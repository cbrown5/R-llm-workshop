...
3. **Model complexity**: Different models have varying capabilities based on their size, training data, and architecture. Bigger models with more training data (e.g., Claude-4.0-Opus vs. Claude-4.0-Sonnet) generally provide more nuanced statistical guidance but at higher computational and financial cost.
...
### Tips for getting better statistical advice from LLMs

When seeking statistical guidance from LLMs, several prompting strategies can significantly improve response quality. Whereas, we have found that many other prompting strategies commonly spruiked by tech commenators are less effective for elicting good statistical advice (e.g. chain of thought, encouragement or talking down to the LLM). 
